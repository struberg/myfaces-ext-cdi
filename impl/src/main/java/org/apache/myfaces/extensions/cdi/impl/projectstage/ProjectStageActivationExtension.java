...
import org.apache.myfaces.extensions.cdi.api.projectstage.ProjectStageActivated;
...
public class ProjectStageActivationExtension  implements Extension {
    ...
    protected void vetoAlternativeTypes(@Observes ProcessAnnotatedType<Object> pat) {
        if (pat.getAnnotatedType().getJavaClass().isAnnotationPresent(ProjectStageActivated.class)) {
            ProjectStage[] activatedIn = pat.getAnnotatedType().getJavaClass().getAnnotation(ProjectStageActivated.class).value();
    ...
    private boolean isInProjectStage(ProjectStage[] activatedIn) {
        if (activatedIn != null && activatedIn.length > 0) {
            ProjectStage ps = getProjectStage();
            for (ProjectStage activated : activatedIn) {
                if (ps.equals(activated)) {
                    return true;
                }
            }
        }
